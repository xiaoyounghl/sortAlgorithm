...
public class QuickSort<E> extends BaseSort<E> {
    ...
    public static void main(String[] args) {
        ...
        QuickSort<Student> hs = new QuickSort<>();

        Comparator<Student> sumC = Comparator.comparingInt(Student::getSum);
        Comparator<Student> subjectC = Comparator.comparingInt(Student::getSubject);
        Comparator<Student> mathC = Comparator.comparingInt(Student::getMath);
        hs.loadComparator(sumC, subjectC, mathC).setOrder(true);
//        hs.loadComparator(sumC).setOrder(true);
//        hs.loadComparator(Integer::compareTo).setOrder(false);
//        List<Integer> arrays;
        List<Student> arrays = new ArrayList<>();
//        if (true) {
//            Integer[] ints = new Integer[]{1, 60, 32, 34, 612, 64, 123, 74, 234, 3, 6, 2, 61, 23, 9, 83};
//                              ints = new Integer[]{82, 60};
//            arrays = Arrays.asList(ints);
        for (int i = 0; i < 50; i++) {
            Student student = new Student();
            student.setSum((int) (Math.random() * 5) + 650); // 750
            student.setSubject((int) (Math.random() * 5) + 300); // 360
            student.setMath((int) (Math.random() * 40) + 80); // 120
            arrays.add(student);
        }
//        } else {
